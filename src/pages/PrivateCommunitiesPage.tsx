 ...
 const buttonPrimary = isDark
  ...
  const fetchCommunities = async () => {
    try {
      ...
      const res = await axios.get(`${BASE_URL}/api/community/private`, {
  ...
  return (
    <div className={`flex min-h-screen ${bgColor} ${textColor} transition-colors duration-300`}>
      <main className="flex-1 overflow-y-auto p-6">
        <h1 className="text-3xl font-bold mb-4">Private Communities</h1>
        <p className={`mb-6 ${subtextColor}`}>Join with a code or invite.</p>
        ...
        <div className="flex flex-wrap gap-4 mb-10">
          ...
          <JoinCommunityModal onJoined={fetchCommunities}>
            <button className={`px-6 py-3 rounded-lg text-sm font-medium transition ${buttonPrimary}`}>
              Join Community
            </button>
          </JoinCommunityModal>
      ...
      >
        <div
          className={`absolute left-0 top-0 h-screen w-64 ${cardBg} shadow-lg overflow-y-auto transform transition-transform duration-300 ${
